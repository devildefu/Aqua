...
unsigned int BAR4) {
   ...
   for (i = 0; i < 2; i++)
      for (j = 0; j < 2; j++) {
         ...
         wait(1); // Wait 1ms for drive select to work.

         // (II) Send ATA Identify Command:
         ide_write(i, ATA_REG_COMMAND, ATA_CMD_IDENTIFY);
         wait(1); // This function should be implemented in your OS. which waits for 1 ms.
   ...
   for (i = 0; i < 4; i++)
      if (ide_devices[i].Reserved == 1) {
        ...
        */
        puts("Found ");
        puts((const char *[]){"ATA", "ATAPI"}[ide_devices[i].Type]);
        puts(" Drive ");
        puts(ide_devices[i].Size / 1024 / 1024 / 2);
        puts("GB - ");
